...
async function drawBadgeTopLeft(page, pdfDoc, userData) {
    ...
    try {
        const montserratRegular = fs.readFileSync('fonts/Montserrat-Regular.ttf');
        const montserratBold = fs.readFileSync('fonts/Montserrat-Bold.ttf');
    ...
    const decalX = 20;
    // 1. Code-barres en haut à gauche
    const codeStr = (badgeCode || '').trim();
    let y = 780;
    if (codeStr) {
