...
export async function generateTicket({
    ...
    try {
        ...
        if (intervenant) {
            page.drawText(`Intervenant : ${intervenant}`, { x: 40, y: 437, size: 10, font: fontRegular, color: rgb(0.2, 0.2, 0.2) });
        }
        page.drawText(new Date(eventDate).toLocaleString(), { x: 40, y: 425, size: 10, font: fontRegular, color: rgb(0.2, 0.2, 0.2) });
        if (salle) {
            page.drawText(`Salle : ${salle}`, { x: 40, y: 412, size: 10, font: fontBold, color: rgb(0.2, 0.2, 0.2) });
        }
