...
const posterUrl = 'https://cnol-badge.vercel.app/cnol2025-poster.jpg';
...
async function generateBadge(userData) {
    try {
        ...
        // Affichage du code d'identification au centre-gauche dans un encadré avec 'ICI'
        if (userData.identifiant_badge) {
            // Encadré
            page.drawRectangle({
                x: 20, y: infoY + 470, width: 120, height: 40, borderColor: rgb(0.1, 0.1, 0.8), borderWidth: 1.5, color: rgb(1,1,1)
            });
            // Code
            page.drawText(userData.identifiant_badge, {
                x: 35, y: infoY + 490, size: 20, font: fontBold, color: rgb(0.1, 0.1, 0.8)
            });
            // Label 'ICI'
            page.drawText('ICI', {
                x: 35, y: infoY + 475, size: 12, font: fontBold, color: rgb(0.1, 0.1, 0.8)
            });
        }
