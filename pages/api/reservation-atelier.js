...
export default async function handler(req, res) {
  ...
  if (inscrit && inscrit.id) {
    const notifBody = type === 'interne'
      ? 'Votre ticket atelier est envoyé par email.'
      : 'Votre réservation atelier est bien prise en compte. Vous recevrez votre ticket par email après validation.';
    await supabase.from('notifications').insert({
      user_id: inscrit.id,
      title: 'Réservation atelier CNOL 2025',
      body: notifBody,
      url: null
    });
    try {
