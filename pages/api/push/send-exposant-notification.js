...
export default async function handler(req, res) {
  ...
  try {
    // Vérifier l'authentification de l'exposant via le cookie de session
    ...
    // Vérifier que l'exposant_id correspond à celui de l'utilisateur connecté
    ...
    // Récupérer le type d'exposant pour déterminer le quota
    ...
    const quotaLimit = quotaLimits[exposantData.type] || 1; // Par défaut 1 si type non reconnu

    // Vérifier le quota de notifications (selon le type d'exposant)
    ...
    // Récupérer TOUS les abonnements push de l'application
    ...
    if (!subscriptions || subscriptions.length === 0) {
      return res.status(404).json({ message: 'Aucun abonnement push trouvé dans l\'application.' });
    }
    ...
    if (!usersError && allUsers) {
      const insertPromises = allUsers.map(async (user) => {
        ...
        if (error) {
          console.error('Erreur insertion notification pour user', user.id, ':', error.message);
        }
      });
