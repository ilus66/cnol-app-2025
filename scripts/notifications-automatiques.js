// Script Node.js pour notifications automatiques
// Usage : node scripts/notifications-automatiques.js
...
async function checkAdminNotifications() {
  ...
  const { data: exposantsQuota } = await supabase
    .from('exposants')
    .select('*')
  ...
  const { data: scansRecents } = await supabase
    .from('leads')
    .select('exposant_id')
    .gte('created_at', yesterday);
  
  const exposantsActifs = [...new Set(scansRecents?.map(s => s.exposant_id) || [])];
  const { data: exposantsInactifs } = await supabase
    .from('exposants')
    .select('*')
...
async function checkNewContacts() {
  ...
  const { data: nouveauxScans } = await supabase
    .from('leads')
    .select(`
      exposant_id,
...
// Fonction principale
async function main() {
  console.log('🚀 Démarrage des notifications automatiques...');
  ...
  try {
    ...
    console.log('✅ Notifications automatiques terminées');
  } catch (error) {
    console.error('❌ Erreur lors des notifications automatiques:', error);
  }
}
